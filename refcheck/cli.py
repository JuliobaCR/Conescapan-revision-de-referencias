...
def process_paper(pdf: Path, verifier: Verifier, grobid_url: str) -> dict:
    ...
    for ref in refs:
        ...
        out.append(row)
    return {"file": pdf.name, "path": str(pdf), "references": out}
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {
            pool.submit(process_paper, p, verifier, args.grobid): p for p in pdfs
        }
        for i, fut in enumerate(as_completed(futures), 1):
            ...
            print(f"  [{i}/{len(pdfs)}] {pdf.name} — "
                  f"{len(res['references'])} refs, {flagged} por revisar")
            results.append(res)
